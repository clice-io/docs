...
> `feature/` 仅涵盖单文件、基于 AST 的功能实现。跨文件代码导航功能（转到定义、查找引用等）由服务器的读取侧服务（`Features`/`IndexQuery`）通过索引数据提供。部分功能涉及多阶段处理——例如，代码补全中的 include 路径补全无需完整编译即可在语法层处理。

### `src/project/` — 磁盘上的项目

一个项目由一组编译数据库和一个缓存目录描述：它是从磁盘文件推导出的状态，独立于编辑器，也独立于调度。命令行只运行一个项目。服务器为每个工作区文件夹运行一个项目；对于在这些文件夹之外打开、且所在目录或某个上级目录中含有 `clice.toml` 或 `compile_commands.json` 的文件，也会再为其运行一个项目，并将每个文件路由到负责编译它的项目。

- `Project`：以磁盘状态为准的聚合体——包含配置、编译数据库、构建视图、依赖图、产物记录和持久化索引。核心不变量：打开文件中未保存的缓冲区内容绝不会修改 `Project`；它只反映磁盘上的状态
- `CommandResolver`：仅依据项目解析文件编译时采用的编译命令——自身条目、头文件所用的宿主命令、默认命令或借来的命令——并合成头文件所需的包含方上下文，负责头文件上下文判定
- `IndexStore`：索引持久化事务——合并、保存、与 CDB 协调——以及索引数据库随索引一并存放的元数据块
- `CDBWatcher`：通过 stat 轮询监视项目的编译数据库，以加载其条目时的那次读取为基准判断是否发生变化
- 项目加载：打开缓存存储和索引，加载构建，扫描依赖图
...
- `IndexPump`：后台调度器，通过 worker 处理需要更新的文件，并采用感知前台负载的预算机制
- `SchedulingStack`：worker 池、任务图及其节点族，以及项目的索引存储与后台索引调度器；服务器和批处理驱动程序都以相同方式组装它们
...
语言服务器的核心运行时，负责将上述各层组装成可运行的服务。它的文件同处一个目录，分为四组。

**协议** — 协议定义。描述主进程与 worker 进程之间以及服务器与客户端之间通信所用的消息格式。包括 Worker 协议（编译/查询/构建请求）、LSP 扩展协议（编译上下文切换等），以及 `clice index` 和 `clice query --fresh` 用来请求运行中的服务器建立索引的控制协议。
...
- `EditorContext`：命令解析中属于编辑器的一侧——用户的上下文选择、为打开文件解析出的头文件上下文，以及这些文件合成 Preamble 所用的宿主——叠加在项目的 `CommandResolver` 之上，仅用于面向编辑器的编译
- `Invalidator`：失效引擎——归并文件事件（缓冲区打开/保存、磁盘内容变化、编译数据库重新加载、worker 崩溃），生成一组去重的失效操作
- `FileTracker`：通过 stat 轮询发现编辑器之外发生的变化（重新生成 `compile_commands.json`、`git checkout`），并将事件送入 `Invalidator`：它包括对项目编译数据库的监视，以及对磁盘文件的一轮扫描——后者以扫描各文件 include 边时所依据的内容为基准，判断文件是否变化
...
- `MasterServer`：组合根。持有文件表、worker 池和所服务的各个项目，并将每个打开的文件路由到负责编译它的项目
- `ProjectServer`：为单个项目提供服务的全部内容——它的调度栈、路由到它的打开文档以及上述服务——并通过自身唯一的分派入口执行 `Invalidator` 产生的操作
- `LSPClient`：LSP 协议的请求处理器
- 控制通道：服务器在持有项目索引写入锁期间开启的回环监听端口，地址记录在锁旁边，供命令行工具查找
