...
一个项目由一组编译数据库和一个缓存目录描述：它是从磁盘文件推导出的状态，独立于编辑器，也独立于调度。目前命令行与服务器都只运行一个项目，服务器在其上再加上打开的缓冲区。
...
- `MasterServer`：组合根。持有项目、会话、工作线程池和上述所有服务，并通过自身唯一的分派入口执行 `Invalidator` 产生的操作
