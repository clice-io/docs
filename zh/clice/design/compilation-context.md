...
- **不支持工作区级别的配置切换**。上下文按文件选择：头文件可以切换宿主，源文件可以在其 CDB 条目之间切换。规则可以声明多个编译数据库，并为它们标注配置（参见 [命令解析](command-resolve.md)），但整个会话期间生效的始终是 `default_configuration` 指定的那个配置；运行时一次切换所有文件（例如，把整个工作区在 Debug 编译数据库与交叉编译数据库之间翻转）尚未实现。如果没有声明任何数据库，发现流程会按名称顺序，取工作区根目录或其一级子目录下找到的第一个 `compile_commands.json`。
