# 功能概览

clice 提供一套基于 LLVM/Clang 构建的 C++ 开发工具。本节介绍已实现和计划中的功能，并提供相关上游问题的链接。

## LSP 编辑器功能

将 clice 用作编辑器后端时可用的语言服务器协议功能。

<!-- The status matrix is generated from the snapshot fixtures under
     tests/snap/. Do not edit the region between the GENERATED markers by
     hand — edit the fixtures (or OVERVIEW_ROWS in tools/docs/feature.ts)
     and run `node tools/docs/feature.ts update`. -->

<!-- BEGIN GENERATED OVERVIEW -->

| 功能       | 状态                                    | 页面                                      |
| ---------- | --------------------------------------- | ----------------------------------------- |
| 代码补全   | 31 项支持                               | [completion](./completion.md)             |
| 悬停       | 34 项支持 · 21 项部分支持 · 11 项不支持 | [hover](./hover.md)                       |
| 签名帮助   | 14 项支持                               | [signature-help](./signature-help.md)     |
| 代码导航   | 44 项支持 · 14 项部分支持 · 34 项不支持 | [navigation](./navigation.md)             |
| 文档链接   | 7 项支持 · 1 项部分支持 · 1 项不支持    | [document-links](./document-links.md)     |
| 语义 Token | 54 项支持 · 4 项部分支持 · 10 项不支持  | [semantic-tokens](./semantic-tokens.md)   |
| 内联提示   | 31 项支持 · 6 项部分支持 · 4 项不支持   | [inlay-hints](./inlay-hints.md)           |
| 折叠范围   | 13 项支持 · 2 项部分支持 · 6 项不支持   | [folding-ranges](./folding-ranges.md)     |
| 文档符号   | 18 项支持 · 2 项部分支持 · 7 项不支持   | [document-symbols](./document-symbols.md) |
| 格式化     | 已实现                                  | [formatting](./formatting.md)             |
| 诊断       | 部分支持                                | [diagnostics](./diagnostics.md)           |
| 代码操作   | 存根                                    | [code-action](./code-action.md)           |

<!-- END GENERATED OVERVIEW -->

## Lint

由 clang-tidy 驱动的项目级静态分析，并采用 clice 独有的跨 TU 优化。

| 功能            | 状态     | 页面              |
| --------------- | -------- | ----------------- |
| clang-tidy 集成 | 部分支持 | [lint](./lint.md) |

项目级 CLI（`clice lint`）已实现；编辑器内实时
诊断和跨 TU 去重仍在计划中。

## 图例

由测试夹具（fixture）支撑的功能，会按状态统计其测试语料库所固定的已记录能力：

- **支持** — 该能力可用；快照（snapshot）固定其行为
- **部分支持** — 尚不完整；快照固定目前可用的行为
- **不支持** — 已记录的功能缺口，已跟踪但尚未实现

尚未纳入 fixture 管线的功能仍使用手动指定的标签：

- **已实现** — 核心功能可用，仅有少量不足
- **部分支持** — 关键子系统缺失（如模块支持）
- **存根** — 处理器已存在，但返回空结果/null
- **计划中** — 已完成设计，但尚未实现
