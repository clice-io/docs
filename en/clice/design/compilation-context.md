...
- **Switching a build configuration restarts the server**. The selection is persisted and applied by the next server start rather than swapped in-process; open documents are re-opened by the client afterwards. Without any declared database, discovery loads the first `compile_commands.json` it finds under the workspace root or its immediate subdirectories, in name order.
