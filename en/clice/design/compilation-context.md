...
- **No workspace-level configuration switching**. Context selection is per file: a header can switch hosts and a source file can switch among its CDB entries, but there is no notion of a named build configuration that switches every file at once (for example, flipping the whole workspace between a Debug and a cross-compilation database). When several compilation databases are discovered, one is chosen deterministically and used alone.
