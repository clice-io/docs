...
- **No workspace-level configuration switching**. Context selection is per file: a header can switch hosts and a source file can switch among its CDB entries. Rules can declare several databases and tag them with configurations (see [Command Resolution](command-resolve.md)), but the configuration named by `default_configuration` stays active for the whole session; switching every file at once at runtime (for example, flipping the whole workspace between a Debug and a cross-compilation database) is not implemented yet. Without any declared database, discovery loads the first `compile_commands.json` it finds under the workspace root or its immediate subdirectories, in name order.
