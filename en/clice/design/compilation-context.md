...
4. **Rule default command**. A file with neither an entry nor a host takes the `default_command` of the first matching rule that declares one — the way to describe a project whose files all share one set of flags.

5. **Borrowed command**. For a file with neither a CDB entry nor any host, and no matching rule with a `default_command` — a freshly created source or a header nothing includes yet — the command of a nearby translation unit of the same language: a unit in the file's directory (same stem first), else the unit whose header search directories (`-I` etc.) contain the header, else the unit closest by path (see [Inference](command-resolve.md#inference)). The command source is labeled `Inferred`, the decision log names the lender, and diagnostics about missing files come with a guidance note. A borrowed command serves the editor only; the file is not a member of the build and does not join the background index.

6. **Builtin fallback**. When everything above falls through, an isolated file compiles with a synthesized builtin command, so basic language services keep working.
...
**`clice/queryContext`** lists all available compilation contexts for a file. For headers, it returns the source files that can serve as hosts, ranked by relevance; for source files, it returns all CDB entries, using key flags from the compilation command (`-D`, `-std`, `-O`, etc.) as human-readable descriptions, each carrying a command hash identifying the entry. Results support pagination. For self-contained headers and source files, the context list is deduplicated by the entry's **identity hash**, computed over the frontend-relevant view of the effective command plus the input's slot position and working directory: two contexts with the same identity produce identical compilation results, so only the best-ranked representative is kept — otherwise a widely-included header like `<vector>` could list thousands of contexts. Non-self-contained headers are not merged this way: different hosts synthesize different prefixes, so every host is a distinct context. When a guard-less header is included several times by one host, each include position is returned as a separate entry distinguished by an occurrence number.
...
Context selection is per file. The workspace-wide counterpart is the **build configuration**: rules in `clice.toml` can carry a `configuration` tag (see [Command Resolution](command-resolve.md)), the distinct tags form a menu, and exactly one of them is active in a server process — its tagged rules apply, untagged rules always apply, and everything a file compiles with follows from that: the databases consulted, the default commands, the edits. The active configuration is resolved once at startup, from three layers in priority order:

1. `--configuration <tag>` on the command line, which pins the session; `clice serve`, `clice index`, `clice lint` and `clice inspect` all take it.
2. The persisted selection: `state.json` under the cache directory, next to the store. It survives cache layout upgrades, which discard the store but not this file.
3. `default_configuration` from the configuration, else the first declared tag.

A name in either of the first two layers that no rule declares is reported as guidance and skipped, and the selection file is left as it is — a renamed tag falls back to the default until the user picks again. That is the server's rule, since an editor must always start; `clice index`, `clice lint` and `clice inspect` run scripted and fail on an undeclared `--configuration` instead of reporting success for another configuration.
...
If the index only records the result from one context, go-to-definition or find-references would miss information from the other context. Each file's index shard therefore stores the rows produced under different compilation contexts as separate variants, and the project index records which translation units contributed them; queries return the union of the live variants — for `config.h`, find-references can find references to both `AsyncHandler` and `SyncHandler`. See [Index Design](symbol-index.md) for the specific merging and deduplication mechanisms.
...
- **Switching a build configuration restarts the server**. The selection is persisted and applied by the next server start rather than swapped in-process; open documents are re-opened by the client afterwards. Without any declared database, discovery loads every `compile_commands.json` at the workspace root and in its immediate subdirectories, and the ones above a file when it is opened; a nested project's database is not scanned for before then.
